<?php
    ...
    if(!empty($post_json)){
        ...
        $job = new Jobs();
        $job->admin_string = !empty($post->admin_string) ? (string)$post->admin_string : "";
        ...
        $job->job_link = !empty($post->job_link) ? (string)$post->job_link : "";
        ...
        $admin = admins::find_by_verify_string($job->admin_string);
        if($admin){
            if($job->insert()){
            ...
            $return_array['status'] = 'failed';
            $return_array['message'] = 'admin does not exist';
        }
    } else {
        $return_array['status'] = 'failed';
