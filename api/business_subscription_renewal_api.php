    ...
    if(!empty($post_json)){
        ...
        if(!empty($verify_string)){
            ...
            if(!empty($sub_payment)){
                ...
                if(!empty($user)){
                    $old_subscribe = Subscribers::find_business_latest_subscription($sub_payment->business_string);
                    if(!empty($old_subscribe)){
                        $time = time();
                        if(($old_subscribe->status == 1) && ($old_subscribe->true_expiry >= $time)){
                            $plan = SubscriptionPaymentPlans::find_by_verify_string($sub_payment->paymentplan_string);
                            if($plan->subscription_string == $old_subscribe->subscription_string){
                                ...
                                $subscribe->started = $old_subscribe->expired;
                                ...
                                if($subscribe->renew()){
                                    ...
                                    $content .=     'Your Subscription Renewal on <a href="https://yenreach.com">Yenreach</a> for your Business, ';
                                ...
                                $return_array['status'] = 'failed';
                                $return_array['message'] = 'Previous Subscription Plan is not same as the one currently being renewed';
                            }
                        } else {
                            $return_array['status'] = 'failed';
                            $return_array['message'] = 'The Subscription is already expired therefore, cannot be renewed';
                        }
                    } else {
                        $return_array['status'] = 'failed';
                        $return_array['message'] = 'There is no previous Subscription to Renew';
                    }
                } else {
                    $return_array['status'] = 'failed';
